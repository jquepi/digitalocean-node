(function() {
  ...
  var ImageAction = (function() {
    ...
    // perPage, optional
    // callback, required
    ImageAction.prototype.list = function(imageId, callback) {
      var id = arguments[0],
          i,
          params = 2 <= arguments.length ? slice.call(arguments, 1, i = arguments.length - 1) : (i = 0, []),
          callback = arguments[i++];

      return this.client.get.apply(this.client, ['/images/' + id + '/actions', {}].concat(slice.call(params), [function(err, status, response, headers) {
        if (err) {
          return callback(err);
        }
        if (status !== 200) {
